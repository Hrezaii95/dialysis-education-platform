...
// ── mastery-level → approximate % for a single learner ─────────────────────
...
/**
 * Derive a competency × KT mastery matrix from real store state.
 *
 * Strategy per cell:
 *  - Base: skill progress for the competency's mappedSkills, weighted by the
 *    competency's declared knowledge-type mix percentages.
 *  - Boost from caseDecisions count (→ conditional) and card retention (all KT).
 *  - Clamp to [0, 100].
 *
 * Returns null when there is genuinely no learner activity so the admin page
 * can fall back to the synthetic cohort.
 */
function deriveHeatmapCells(
  ...
  // Detect real activity: any skill touched, any card reviewed, any case decision, any xAPI event
  const hasActivity =
  ...
  for (const comp of COMPETENCIES) {
    // 1. Get the best skill level for this competency (own key + mapped skills)
    ...
    // 2. Card retention boost: each retained review = +5%, capped at +20%
    const card = cards[comp.id];
    const retentionBoost = card ? Math.min(card.retained * 5, 20) : 0;

    // 3. xAPI events for this competency: use path_rewritten "weak" signals as
    //    negative indicators; "mastered" events as positive.
    ...
    // answered statements: fraction correct
    ...
    // 4. Case-decision signal for conditional KT
    const caseBonus = caseDecisions.length > 0 ? Math.min(caseDecisions.length * 3, 15) : 0;

    // 5. Compute per-KT values using the competency's declared knowledge mix
    //    as a weight: a competency with knowledge.declarative=80 concentrates
    //    its skill progress mostly into the declarative cell.
    ...
    // A KT that the competency doesn't exercise (weight=0) gets 0; others get
    // scaled versions of the base score, then the conditional cell also gets the
    // case-decision boost.
...
function cellColor(pct: number) {
  ...
  if (pct < 80) return "bg-teal/15 text-teal";
  return "bg-teal/30 text-teal";
}
...
export default function AdminPage() {
  ...
  // ── live heatmap (or synthetic fallback) ──────────────────────────────────
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard icon={Users}         label="Skills mastered"    value={`${metrics.mastered}/${metrics.totalSkills}`} />
        <MetricCard icon={Clock}         label="Avg alarm response" value={`${(metrics.avgLatency / 1000).toFixed(1)}s`} />
        <MetricCard icon={AlertTriangle} label="Sim interactions"   value={String(metrics.interactions)} />
        <MetricCard icon={Award}         label="Credentials issued" value={String(metrics.credentials)} />
      </div>
      ...
      <section className="glass-panel p-5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span className="text-[10px] uppercase tracking-wide text-muted">
            {heatmap.isSynthetic ? (
              <span className="text-gold">synthetic cohort (demo)</span>
            ) : (
              <span className="text-teal">live learner data</span>
            )}
            {" · "}
            {heatmap.isSynthetic
              ? `n=${heatmap.size} · ${t("admin.noPii", "no PII")}`
              : `n=1 learner · ${t("admin.noPii", "no PII")}`}
          </span>
        </div>
        ...
        {heatmap.isSynthetic && (
          <p className="mt-2 text-[10px] text-muted">
            Showing synthetic demo data — heatmap updates automatically once a learner completes
            activities (skills, cases, cards, or the credential assessment).
          </p>
        )}
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="glass-panel p-5">
          <h3 className="font-semibold mb-4">Competency by skill</h3>
        ...
        <div className="glass-panel p-5">
          <h3 className="font-semibold mb-4">Recent xAPI events</h3>
          <div className="max-h-80 overflow-y-auto space-y-2 text-xs font-mono">
            {stmts.length === 0 && (
              <p className="text-muted">No events yet — complete simulator activities</p>
            )}
      ...
      <div className="glass-panel p-5">
        <h3 className="font-semibold mb-2">Case decision log (learner)</h3>
        <ol className="list-decimal list-inside text-sm text-muted space-y-1">
          {caseDecisions.length === 0 && <li>No case decisions recorded</li>}
      ...
      <p className="text-xs text-muted text-center">
        Multi-tenant hospital branding and LMS grade passback available on request
      </p>
    </div>
  );
}
